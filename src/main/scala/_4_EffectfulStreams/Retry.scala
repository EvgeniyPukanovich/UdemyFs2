  ...
  override def run: IO[Unit] = {
    ...
    def doEffectFailing[A](io: IO[A]): IO[A] = {
      IO(math.random()).flatMap { flag =>
        if (flag < 0.9) IO.println("Failing...") *> IO.raiseError(new Exception("boom"))
    ...
    // Exercise
    val searches = Stream.iterateEval("")(s => IO.println(s"Searching $s") >> IO(Random.nextPrintableChar()).map(s + _))
    ...
    def performSearchRetrying(text: String): Stream[IO, Unit] = Stream.retry(

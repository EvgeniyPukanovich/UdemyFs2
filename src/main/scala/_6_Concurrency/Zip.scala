...
object Zip extends IOApp.Simple {
  ...
  override def run: IO[Unit] = {
    ...
    val s2Inf = Stream.iterate(start = 0)(_ + 1).covary[IO].metered(200.millis)
    ...
    s2.zip(s1).compile.toList.flatMap(IO.println)
    ...
    val s4 = Stream.iterateEval(start = 0)(i => IO.println(s" Pulling left $i").as(i + 1))
    val s5 = Stream.iterateEval(start = 0)(i => IO.println(s" Pulling right $i").as(i + 1))
